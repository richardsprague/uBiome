...
#            self.samples+=[[newSample.name]+[sample["count_norm"] for sample in newSample.sampleList]]
    ...
    def merge(self,sample2,mergeField="count_norm"):
        ...
        :type UbiomeSample sample2: sample to merge with
        :return: None
        """

        if len(self.fullTaxList)==1: #special case when there are no existing samples
            self.fullTaxList +=sample2.taxnames()
            self.samples+=[[sample2.name]+[getattr(sample,mergeField) for sample in sample2.taxaList]]
            self.originalSampleObjects = [sample2]
            return True
        ...
        for i in range(len(oldSamplesList)-1):
            ...
            if taxonForTaxName:
                taxCount = getattr(taxonForTaxName,mergeField)
            else: taxCount = 0
            newSampleCountsForPreviousTaxa+=[taxCount]
        newCounts =  newSampleCountsForPreviousTaxa + [getattr(taxon,mergeField) for taxon in newTaxons]
